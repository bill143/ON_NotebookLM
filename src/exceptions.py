...
def classify_error(error: Exception) -> tuple[type[NexusError], str]:
    ...
    if any(term in error_str for term in ["unauthorized", "401", "invalid api key", "authentication"]):
        return ProviderAuthError, "AI service authentication failed. Check your API key configuration."
    ...
    # Connection
    if any(term in error_str for term in ["connection refused", "connection error", "dns", "network"]):
